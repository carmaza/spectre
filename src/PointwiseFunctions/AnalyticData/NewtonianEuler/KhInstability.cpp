...
KhInstability<Dim>::KhInstability(
    const double adiabatic_index, const double strip_bimedian,
    ...
    : adiabatic_index_(adiabatic_index),
      strip_bimedian_(strip_bimedian),
  ...
  ASSERT(strip_density_ > 0. and background_density_ > 0.,
         "The mass density must be positive everywhere. Inner "
         "density: "
             << strip_density_ << ", Outer density: " << background_density_
             << ".");
  ASSERT(pressure_ > 0., "The pressure must be positive. The value given was "
                             << pressure_ << ".");
  ASSERT(damping_factor_ > 0.,
         "The damping factor must be positive. The value given was "
             << damping_factor_ << ".");
  ASSERT(strip_thickness > 0.,
...
tuples::TaggedTuple<Tags::MassDensity<DataType>> KhInstability<Dim>::variables(
  ...
  for (size_t s = 0; s < n_pts; ++s) {
    get_element(get(result), s) = abs(get_element(get<Dim - 1>(x), s) -
                                      strip_bimedian_) < strip_half_thickness_
                                      ? strip_density_
                                      : background_density_;
  }
...
KhInstability<Dim>::variables(
  ...
  for (size_t s = 0; s < n_pts; ++s) {
    get_element(get<0>(result), s) =
        abs(get_element(get<Dim - 1>(x), s) - strip_bimedian_) <
                strip_half_thickness_
            ? strip_velocity_
            : background_velocity_;
  }

  const double one_over_two_sigma_squared = 0.5 / square(damping_factor_);
  const double strip_lower_bound = strip_bimedian_ - strip_half_thickness_;
  const double strip_upper_bound = strip_bimedian_ + strip_half_thickness_;
...
bool operator==(const KhInstability<Dim>& lhs,
  ...
  return lhs.adiabatic_index_ == rhs.adiabatic_index_ and
         lhs.strip_bimedian_ == rhs.strip_bimedian_ and
         ...
         lhs.damping_factor_ == rhs.damping_factor_;
}
